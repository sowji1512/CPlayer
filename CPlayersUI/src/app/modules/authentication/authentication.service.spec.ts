...
fdescribe('AuthenticationService', () => {
  ...
  it('should register user data', fakeAsync(() => {
    ...
    inject([AuthenticationService, HttpTestingController], (backend: HttpTestingController) => {
      const mockReq = backend.expectOne(authService.serviceEndpoint);
      expect(mockReq.request.url).toEqual(authService.serviceEndpoint, 'request url should match with json server api url');
